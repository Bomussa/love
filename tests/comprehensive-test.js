...
async function testRealTimeNotifications() {
  ...
  try {
    ...
    for (const pos of positions) {
      ...
      logTest(
        'Notifications',
        `Position ${pos} Notification Trigger`,
        shouldNotify,
...
async function testPINSystem() {
  ...
  try {
    ...
    if (pinData.pins) {
      const allValid = Object.values(pinData.pins).every((pin) => typeof pin === 'number' && pin >= 10 && pin <= 99);
