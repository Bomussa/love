...
const DEV_PIN_FALLBACK_FLAG =
  (typeof import.meta !== 'undefined' && import.meta?.env?.VITE_ENABLE_LOCAL_PIN_FALLBACK === 'true')
  || (typeof process !== 'undefined' && process?.env?.VITE_ENABLE_LOCAL_PIN_FALLBACK === 'true');

const isProductionRuntime =
  (typeof import.meta !== 'undefined' && import.meta?.env?.PROD)
  || (typeof process !== 'undefined' && process?.env?.NODE_ENV === 'production');

const canUseLocalPinFallback = () => !isProductionRuntime && DEV_PIN_FALLBACK_FLAG;
...
class APIAdapter {
  ...
  async issuePin(clinicId, visitId = null) {
    ...
    if (this.localApi && canUseLocalPinFallback()) {
      return this.localApi.generatePin(clinicId);
    }

    throw new Error('PIN local fallback is disabled in production. Set VITE_ENABLE_LOCAL_PIN_FALLBACK=true for development only.');
  }
