...
class EnhancedApiClient {
  ...
  /**
     * Get queue position for patient/session
     * Backend: POST /api/v1/queue/position
     * Body: { clinicId, patientId?, sessionId? }
     */
  async getQueuePosition(clinicId, patientId, sessionId = null) {
    return this.request(`${API_VERSION}/queue/position`, {
      method: 'POST',
      body: JSON.stringify({
        clinicId,
        patientId,
  ...
  playNotificationSound(type = 'info') {
    try {
      ...
      console.warn('[EnhancedAPI] audio alert failed:', error);
    }
  }
