...
export async function addToQueue(supabase, patientData) {
  ...
  const nextDisplayNumber = lastEntry ? lastEntry.display_number + 1 : 1;
...
export async function getPatientPosition(supabase, patientId) {
  ...
  if (error) {
    throw new Error(`Failed to get patient queue position: ${error.message}`);
  }
...
export async function getClinicStats(supabase, clinicId) {
  ...
  const stats = {
    ...
    serving: queue.filter((p) => p.status === 'serving' || p.status === 'called').length,
