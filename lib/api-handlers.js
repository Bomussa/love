...
function normalizeCanonicalPatientId(value) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (pathname === '/api/v1/patient/login' && method === 'POST') {
      const personalId = normalizeCanonicalPatientId(body.personalId ?? body.patientId);
      const { gender } = body;
    ...
    if (pathname === '/api/v1/queue/enter' && method === 'POST') {
      ...
      const { data: rpcResult, error: rpcError } = await supabase.rpc('enter_unified_queue_safe', rpcArgs);
      if (rpcError || !Array.isArray(rpcResult) || rpcResult.length === 0) {
        return res.status(503).json(formatError('Queue enter unavailable', 'QUEUE_RPC_UNAVAILABLE'));
      }

      const row = rpcResult[0];
      const queueLength = row.queue_length ?? row.position ?? row.display_number ?? 1;

      return res.status(200).json(formatSuccess({
        position: row.position ?? row.display_number,
        queueLength,
        estimatedWait: Number(row.position ?? row.display_number ?? 1) * 5,
        alreadyInQueue: Boolean(row.already_exists),
      }, row.already_exists ? 'Already in queue' : 'Successfully entered queue'));
    }
    ...
    if (pathname === '/api/v1/queue/status' && method === 'GET') {
      ...
      const { data: queueRows, error } = await supabase
        .from('queues')
        .select('display_number,status,entered_at')
        .eq('clinic_id', clinicId)
        .eq('queue_date', today)
        .order('display_number', { ascending: true });

      if (error) {
        return res.status(503).json(formatError('Queue status unavailable', 'QUEUE_STATUS_UNAVAILABLE'));
      }

      const waitingRows = (queueRows || []).filter((row) => row.status === 'waiting');
      const servingRow = (queueRows || []).find((row) => row.status === 'serving' || row.status === 'called');

      return res.status(200).json(formatSuccess({
        clinicId,
        queueLength: waitingRows.length,
        currentNumber: servingRow ? servingRow.display_number : 0,
        patients: waitingRows.map((row, index) => ({
          position: index + 1,
    ...
    if (pathname === '/api/v1/pin/verify' && method === 'POST') {
      ...
      const dayStart = `${useDateKey}T00:00:00.000Z`;
      const dayEnd = `${useDateKey}T23:59:59.999Z`;

      const { data: pinData, error } = await supabase
        .from('pins')
        .select('clinic_id,pin,valid_until,used_at,created_at')
        .eq('clinic_id', clinicId)
        .eq('pin', String(pin))
      ...
      if (pinData.valid_until && new Date(pinData.valid_until) < new Date()) {
        return res.status(401).json(formatError('PIN expired', 'PIN_EXPIRED'));
      }

      return res.status(200).json(formatSuccess({
        valid: true,
        clinicId: pinData.clinic_id,
        dateKey: useDateKey,
      }));
    }
