...
function getRolePermissions(role) {
  const map = {
    SUPER_ADMIN: ['*'],
    ADMIN: ['admin:read', 'admin:write', 'queue:manage', 'reports:read'],
    DOCTOR: ['queue:read', 'queue:update'],
    RECEPTIONIST: ['queue:read', 'queue:create'],
    VIEWER: ['queue:read'],
  };

  return map[role] || ['queue:read'];
}

export default async function handler(req, res) {
  // Set CORS headers
  const corsResult = setCorsHeaders(res, req);
  if (!corsResult.allowed) {
    return res.status(403).json(formatError('Origin not allowed', 'CORS_ORIGIN_DENIED'));
  }
  ...
  try {
    ...
    if (pathname === '/api/v1/admin/session/verify' && method === 'POST') {
      const { sessionToken } = body;

      if (!sessionToken) {
        return res.status(400).json(formatError('Missing session token', 'MISSING_SESSION_TOKEN'));
      }

      const session = await KV_ADMIN.get(`session:${sessionToken}`);
      if (!session) {
        return res.status(401).json(formatError('Session not found', 'INVALID_SESSION'));
      }
    ...
    if (pathname === '/api/v1/admin/session/verify' && method === 'POST') {
      const sessionToken = body?.sessionToken || body?.sessionId || req.headers['x-session-token'];
      ...
      const sessionExpiresAt = dbSession.expiresAt || dbSession.expires_at;
      if (!sessionExpiresAt || new Date(sessionExpiresAt) <= new Date()) {
        await KV_ADMIN.del(`session:${sessionToken}`);
      ...
      const { data: admin, error: adminError } = await supabase
        .from('admins')
        .select('id, username, role, permissions, name')
        .eq('id', dbSession.userId || dbSession.user_id)
        .single();

      if (adminError || !admin) {
        return res.status(401).json(formatError('Admin account not found', 'ADMIN_NOT_FOUND'));
      }

      const trustedPermissions = Array.isArray(admin.permissions)
        ? admin.permissions
        : ['*'];
      ...
      await KV_ADMIN.put(`session:${sessionToken}`, trustedSession, {
        expirationTtl: Math.max(1, Math.floor((new Date(sessionExpiresAt).getTime() - Date.now()) / 1000)),
      });

      return res.status(200).json(formatSuccess({
        sessionToken,
        username: trustedSession.username,
        role: trustedSession.role,
        permissions: trustedSession.permissions,
    ...
    if (pathname === '/api/v1/queue/position' && method === 'POST') {
      // Canonical contract:
      // POST /api/v1/queue/position
      // Body: { clinicId, patientId? | sessionId? }
      // Transitional aliases still supported from older clients: clinic, user
      const clinicId = body.clinicId || body.clinic || query.clinicId || query.clinic;
      const patientId = body.patientId || body.user || query.patientId || query.user;
      ...
      try {
        ...
        const { data: queueEntries, error } = await supabase
          .from('unified_queue')
          .select('patient_id, session_id, display_number, status, entered_at')
        ...
        if (patientId) {
          position = queueEntries.findIndex((q) => q.patient_id === patientId);
          if (position >= 0) {
            displayNumber = queueEntries[position].display_number;
          }
        } else if (sessionId) {
          position = queueEntries.findIndex((q) => q.session_id === sessionId);
          if (position >= 0) {
            displayNumber = queueEntries[position].display_number;
          }
        }

        const ahead = position >= 0 ? position : null;
        ...
        return res.status(200).json(formatSuccess({
          ...
          total_waiting: queueEntries.length,
          estimatedWaitMinutes: ahead !== null ? ahead * 5 : null,
          inQueue: position >= 0,
        }));
      } catch (err) {
