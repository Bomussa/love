...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (['/api/health', '/api/healthz', '/api/v1/health', '/api/v1/health/status'].includes(pathname) && method === 'GET') {
      return res.status(200).json(formatSuccess({
        status: 'ok',
        service: 'api',
        timestamp: new Date().toISOString(),
    ...
    if (pathname === '/api/v1/qa/deep_run' && (method === 'GET' || method === 'POST')) {
      ...
      const runRecord = {
        run_type: 'deep_run',
        ok: true,
        summary: 'Automated QA deep run',
        created_at: now,
      };

      try {
        await supabase.from('qa_runs').insert(runRecord);
      } catch (error) {
        // qa tables may not exist in all envs
      }

      const { data: clinics } = await supabase.from('clinics').select('id, name_ar, is_active').eq('is_active', true);
      ...
      for (const clinic of activeClinics) {
        ...
        if (!pin) {
          const legacy = await supabase
            ...
            .limit(1)
            .maybeSingle();
          if (!legacy.error && legacy.data) pin = legacy.data;
        }
        ...
        if (!pin) {
          findings.push({
            ...
            severity: 'high',
            clinic_id: clinic.id,
            description: `Missing active PIN for clinic ${clinic.id}`,
          });
          ...
          if (method === 'POST') {
            ...
            const canonicalInsert = await supabase.from('pins').insert({
              clinic_id: clinic.id,
              pin: newPin,
      ...
      return res.status(200).json(formatSuccess({
        ...
        findings,
        fixes,
        mode: method === 'POST' ? 'repair' : 'scan',
        timestamp: now,
      }));
    }

    if (pathname === '/api/v1/repair/execute' && method === 'POST') {
      const repairToken = process.env.REPAIR_EXEC_TOKEN || process.env.VITE_REPAIR_EXEC_TOKEN || null;
      const { findingId, token } = body || {};

      if (repairToken && token !== repairToken) {
        return res.status(401).json(formatError('Invalid repair token', 'INVALID_REPAIR_TOKEN'));
      }

      if (!findingId) {
        return res.status(400).json(formatError('Missing findingId', 'MISSING_FINDING_ID'));
      }
      ...
      if (updateResult.error || !updateResult.data) {
        return res.status(404).json(formatError('Finding not found', 'FINDING_NOT_FOUND'));
      }
