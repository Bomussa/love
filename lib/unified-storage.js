...
class UnifiedStorage {
  ...
  addPatient(data) {
    ...
    const patient = {
      ...
      // NOTE: لا يتم تخزين الجنس (gender) في localStorage لحماية البيانات الحساسة
  ...
  updatePatient(id, updates) {
    ...
    if (index !== -1) {
      // منع تحديث أو إضافة حقل الجنس (gender) في التخزين المحلي
      const { gender, ...safeUpdates } = updates || {};
      patients[index] = { ...patients[index], ...safeUpdates };
      localStorage.setItem('mms_patients', JSON.stringify(patients));
