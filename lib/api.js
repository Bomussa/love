...
class ApiService {
  ...
  async syncOfflineQueue() {
    try {
      ...
      if (remaining.length === 0) {
        ...
        console.warn(`⚠️ Offline queue has ${remaining.length} pending operations`);
      }
    } catch (e) {
   ...
   * Backend: POST /api/v1/queue/position
  ...
  async getQueuePosition(clinic, user, retryCount = 0) {
    ...
    try {
      const data = await this.request(`${API_VERSION}/queue/position`, {
        method: 'POST',
        body: JSON.stringify({
          clinicId: clinic,
          patientId: user,
  ...
  async adminLogin(code) {
    const payload = typeof code === 'string' && code.includes(':')
      ? (() => {
    ...
    for (const base of API_BASES) {
      try {
        const response = await fetch(`${base}${API_VERSION}/admin/login`, {
          method: 'POST',
    ...
    const isUnavailable = !lastError
      || lastError instanceof TypeError
      || /fetch|network|failed to fetch/i.test(String(lastError?.message || ''));
