...
import { localDateKeyAsiaQatar, nowISO } from '../utils/time';
...
const filePath = (clinicId: string, dateKey: string) => 
...
export async function assignTicket(
  ...
  visitId: string, 
  issuedAt?: string
): Promise<{ ticket: number; clinicId: string; dateKey: string; floorHint?: string }> {
  const day = localDateKeyAsiaQatar();
  const file = filePath(clinicId, day);
  const q = await readJSON<QueueFile>(file, {
...
export async function markDone(clinicId: string, visitId: string, ticket: number): Promise<void> {
  const day = localDateKeyAsiaQatar();
  const file = filePath(clinicId, day);
  const q = await readJSON<QueueFile>(file, {
    meta: { clinicId, dateKey: day, version: 1 },
  ...
  const inIdx = q.in.findIndex(x => x.ticket === ticket && x.visitId === visitId);
  if (inIdx >= 0) {
    const [rec] = q.in.splice(inIdx, 1);
    q.done.push({ ticket: rec.ticket, visitId: rec.visitId, doneAt: nowISO() });
  } else {
    const wIdx = q.waiting.findIndex(x => x.ticket === ticket && x.visitId === visitId);
...
export async function getQueueStatus(clinicId: string): Promise<QueueFile> {
  const day = localDateKeyAsiaQatar();
  const file = filePath(clinicId, day);
  return await readJSON<QueueFile>(file, {
    meta: { clinicId, dateKey: day, version: 1 },
    nextCallTicket: 1,
    waiting: [],
    in: [],
