...
export const queueQueries = {
  ...
  async getStatus(clinicId) {
    ...
    const waiting = data.filter((q) => String(q.status || '').toLowerCase() === 'waiting');
    const serving = data.filter((q) => String(q.status || '').toLowerCase() === 'serving');
    const completed = data.filter((q) => String(q.status || '').toLowerCase() === 'completed');
    const skipped = data.filter((q) => String(q.status || '').toLowerCase() === 'skipped');
