  ...
  useEffect(() => {
    const loadPathway = async () => {
      try {
        ...
        // Initialize stations state - will be updated with real data from Supabase
        const initialStations = sortedStations.map((station, index) => ({
          ...station,
          status: index === 0 ? 'ready' : 'locked',
          current: null, // Will be populated from Supabase
          yourNumber: null, // Will be populated from Supabase
          ahead: null, // Will be populated from Supabase
          isEntered: false
        }));
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center px-2 py-3 bg-gray-800/30 rounded-xl" data-test="queue-info">
                      ...
                      <div className="p-3 bg-yellow-500/20 rounded-lg border-2 border-yellow-500/50">
                        <div className="text-3xl font-bold text-yellow-400 mb-1" data-test="your-number">
                          {station.yourNumber ?? patientData.queueNumber ?? '—'}
                        </div>
                        ...
                        <div className="text-center space-y-3">
                          <div className="p-4 bg-yellow-500/20 border border-yellow-500/50 rounded-xl">
                            ...
                            <p className="text-yellow-200 text-sm mt-2">
                              {language === 'ar' 
                                ? `رقمك ${station.yourNumber ?? '—'} - الحالي ${station.current ?? '—'} - أمامك ${station.ahead ?? '—'} شخص`
                                : `Your # ${station.yourNumber ?? '—'} - Current ${station.current ?? '—'} - ${station.ahead ?? '—'} ahead`}
                            </p>
                          </div>
                          <Button
                            variant="outline"
