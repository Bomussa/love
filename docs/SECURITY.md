...
## إدارة الأسرار ومنع التسريب

- جميع المفاتيح الحساسة يجب أن تُدار من خلال متغيرات بيئة فقط (`.env` محليًا، وVercel/Supabase Secrets في البيئات السحابية).
- يُمنع وضع أي قيمة فعلية داخل المستودع. استخدم placeholders مثل:
  - `<SUPABASE_SERVICE_ROLE_KEY>`
  - `<SUPABASE_JWT_SECRET>`
  - `<VERCEL_TOKEN>`
  - `<GITHUB_TOKEN>`
- تم اعتماد فحص أسرار إجباري قبل الالتزام وعبر CI باستخدام `gitleaks`.

### تدوير المفاتيح (Rotation) عند الاشتباه بالتسريب

1. ألغِ/دوّر فورًا: GitHub PAT، Vercel Token، وSupabase Service Role/JWT Secret.
2. حدّث الأسرار الجديدة في:
   - Vercel Project Environment Variables
   - Supabase Secrets
   - بيئات CI/CD
3. لا تُدخل القيم الجديدة في أي ملف داخل المستودع.

### تنظيف التاريخ (History Rewrite) وفق سياسة الفريق

عند وجود مفاتيح تم رفعها سابقًا، نفّذ تنظيف التاريخ بموافقة الفريق الأمني باستخدام أدوات مثل `git filter-repo` أو BFG، ثم:
- force-push للفروع المتأثرة,
- إبطال كل الأسرار القديمة,
- إعادة تشغيل الفحوص الأمنية على كامل الشجرة.
