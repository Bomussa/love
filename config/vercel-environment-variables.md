...
## متطلبات إلزامية لتشغيل الواجهة الأمامية (Production + Preview)

> ابتداءً من الإصلاح الحالي، لن يبدأ التطبيق إذا كانت متغيرات Supabase الخاصة بـ Vite غير موجودة.

يجب ضبط المتغيرين التاليين **في Vercel Production وPreview معًا**:

| المتغير | Production | Preview | ملاحظات |
|---------|------------|---------|---------|
| `VITE_SUPABASE_URL` | إلزامي | إلزامي | رابط مشروع Supabase (HTTPS) |
| `VITE_SUPABASE_ANON_KEY` | إلزامي | إلزامي | مفتاح Anon العام (بدون Placeholder) |

### آلية التحقق عند الإقلاع

- إذا غاب أي متغير من المتغيرين أعلاه، سيُظهر التطبيق شاشة خطأ ودية للمستخدم بدل شاشة تحميل صامتة.
- تظهر رسالة إدارية واضحة في Console لتوجيه الفريق لإصلاح إعدادات Vercel ثم إعادة النشر.
