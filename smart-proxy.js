...
const HEALTH_PATH = process.env.HEALTH_PATH || '/api/v1/status';
...
async function proxyOnce(req, res, attempt = 0, chosenUpstream = null) {
  ...
  if (!ok) {
    ...
    logLine({
      ts: new Date().toISOString(), req_id: reqId, method: req.method, path: req.url, status: 502, dur_ms: Date.now() - started, upstream, retry: attempt, err: `private_ip:${ip}`,
    });
    return;
  }
  ...
  try {
    ...
    logLine({
      ts: new Date().toISOString(), req_id: reqId, method: req.method, path: target.pathname + target.search, status: r.status, dur_ms: Date.now() - started, upstream, retry: attempt, client_auth_hash: clientAuthHash,
    });
  } catch (e) {
    clearTimeout(timeout);
    ...
    res.end('Bad Gateway');
    logLine({
      ts: new Date().toISOString(), req_id: reqId, method: req.method, path: req.url, status: 502, dur_ms: Date.now() - started, upstream, retry: attempt, err: String(e),
    });
  }
}
