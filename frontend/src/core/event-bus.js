    ...
    if (this.listeners.has(event)) {
      for (const callback of this.listeners.get(event)) {
        try {
          ...
          // console.warn(`[EventBus] Error in listener for ${event}:`, error)
        }
      }
    }
    ...
    if (this.listeners.has('*')) {
      for (const callback of this.listeners.get('*')) {
        try {
          ...
          // console.warn(`[EventBus] Error in wildcard listener:`, error)
        }
      }
    }
  ...
  try {
    ...
    sseConnection.addEventListener('queue_update', (e) => {
      try {
        ...
        // console.warn('[EventBus] Error parsing queue_update:', err);
      }
    });
    ...
    sseConnection.addEventListener('queue_call', (e) => {
      try {
        ...
        // console.warn('[EventBus] Error parsing queue_call:', err);
      }
    });
    ...
    sseConnection.addEventListener('notice', (e) => {
      try {
        ...
        // console.warn('[EventBus] Error parsing notice:', err);
      }
    });
    ...
    sseConnection.addEventListener('stats_update', (e) => {
      try {
        ...
        // console.warn('[EventBus] Error parsing stats_update:', err);
      }
    });

    sseConnection.onerror = (err) => {
      // console.warn('[EventBus] ❌ SSE Error:', err);
    ...
    // console.warn('[EventBus] Failed to create SSE connection:', err);
  }
}
