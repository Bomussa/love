  ...
  const generateAllPINs = async () => {
    try {
      ...
      for (const clinic of clinics) {
        try {
          const pinRes = await api.generatePIN(clinic.id)
          if (pinRes.success) {
            newPins[clinic.id] = {
              pin: pinRes.pin,
          ...
          console.error(`Error generating PIN for ${clinic.id}:`, err)
        }
      }
      
      setPins(newPins)
      setLastUpdate(new Date())
      alert(isRTL ? 'تم إنشاء جميع الأرقام بنجاح' : 'All PINs generated successfully')
    } catch (err) {
  ...
  const printPINList = () => {
    const printContent = `
      ...
      <body>
        ...
        <div class="footer">
          ${isRTL ? 'المركز الطبي المتخصص العسكري - العطار - اللجنة الطبية' : 'Military Specialized Medical Center - Al-Attar - Medical Committee'}
          <br>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          ...
          {isRTL ? 'قائمة الأرقام السرية' : 'PIN List'}
        </h2>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-red-600 rounded-lg text-white hover:bg-red-700 transition-colors"
        >
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-800">
              <tr>
                <th className="px-4 py-3 text-left text-white">{isRTL ? 'رقم' : '#'}</th>
                <th className="px-4 py-3 text-left text-white">{isRTL ? 'اسم العيادة' : 'Clinic Name'}</th>
                <th className="px-4 py-3 text-left text-white">{isRTL ? 'رمز' : 'Code'}</th>
                <th className="px-4 py-3 text-left text-white">{isRTL ? 'رقم PIN' : 'PIN'}</th>
                <th className="px-4 py-3 text-left text-white">{isRTL ? 'الحالة' : 'Status'}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-600">
              {clinics.map((clinic, idx) => {
                const pinData = pins[clinic.id]
                return (
                  <tr key={clinic.id} className="hover:bg-gray-600 transition-colors">
                    <td className="px-4 py-3 text-gray-300">{idx + 1}</td>
                    <td className="px-4 py-3 text-white font-medium">
                      {isRTL ? clinic.name_ar : clinic.name_en}
                    </td>
                    <td className="px-4 py-3 text-gray-300">{clinic.id}</td>
                    <td className="px-4 py-3">
                      {pinData?.pin ? (
                        ...
                        <span className="text-gray-500">{isRTL ? 'غير متوفر' : 'N/A'}</span>
                      )}
                    </td>
                    <td className="px-4 py-3">
      ...
      {lastUpdate && (
        <p className="text-sm text-gray-400 text-center">
          {isRTL ? 'آخر تحديث' : 'Last updated'}: {lastUpdate.toLocaleString(isRTL ? 'ar-SA' : 'en-US')}
        </p>
      )}
    </div>
  )
