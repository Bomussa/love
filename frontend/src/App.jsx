...
function App() {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'clinic_login': return <ClinicLoginPage onLogin={(session) => { setClinicSession(session); localStorage.setItem('mmc_clinic_session', JSON.stringify(session)); }} language={language} />
      case 'doctor': return <DoctorDashboard doctorData={doctorSession} onLogout={handleLogout} language={language} />
