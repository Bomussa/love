...
describe('apiClient', () => {
  ...
  it('retries 5xx GET without self-referencing dedupe promise', async () => {
    fetch
      .mockImplementationOnce(() => mockJsonResponse({ error: 'temporary' }, 503))
      .mockImplementationOnce(() => mockJsonResponse({ data: { ok: true } }, 200));

    const data = await apiClient.get('settings', { b: 2, a: 1 });

    expect(fetch).toHaveBeenCalledTimes(2);
    expect(data).toEqual({ ok: true });
  });

  it('dedupe key is stable when GET params order differs', async () => {
    fetch.mockImplementation(() => mockJsonResponse({ data: [{ id: 2 }] }));

    const p1 = apiClient.get('clinics', { a: 1, b: 2 });
    const p2 = apiClient.get('clinics', { b: 2, a: 1 });
