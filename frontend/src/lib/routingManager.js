...
export async function pickClinicForNextStep(examType, gender, currentStep = 1) {
  try {
    ...
    // console.warn('Error picking clinic for next step:', error);
    return null;
  }
}
...
export async function markDistributed(clinicId) {
  try {
    ...
    // console.warn(`Error marking distributed for clinic ${clinicId}:`, error);
    return false;
  }
}
...
export async function getExamRoute(examType, gender) {
  try {
    ...
    // console.warn(`Error getting exam route for ${examType}/${gender}:`, error);
    return [];
  }
}
...
export async function createPatientRoute(patientId, examType, gender) {
  ...
  try {
    ...
    // console.warn(`Error creating patient route for ${patientId}:`, error);
    return false;
  } finally {
    client.release();
  }
...
export async function moveToNextStep(patientId) {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    // console.warn(`Error moving to next step for patient ${patientId}:`, error);
    return null;
  } finally {
    client.release();
  }
...
export async function getPatientRouteStatus(patientId) {
  try {
    ...
    // console.warn(`Error getting route status for patient ${patientId}:`, error);
...
export async function getDistributionStats() {
  try {
    ...
    // console.warn('Error getting distribution stats:', error);
    return [];
  }
}
