...
export async function getQueueSnapshot(clinicId) {
  try {
    ...
    // console.warn(`Error getting queue snapshot for clinic ${clinicId}:`, error);
...
export async function getQueueDetails(clinicId) {
  try {
    ...
    // console.warn(`Error getting queue details for clinic ${clinicId}:`, error);
    return {
...
export async function callNextPatient(clinicId) {
  ...
  try {
    ...
    // console.warn(`Error calling next patient for clinic ${clinicId}:`, error);
    return {
      success: false,
...
export async function checkInAtClinic(clinicId, patientId) {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    // console.warn(`Error checking in patient ${patientId} at clinic ${clinicId}:`, error);
    return false;
  } finally {
    client.release();
  }
...
export async function completeClinicForPatient(clinicId, patientId) {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    // console.warn(`Error completing clinic for patient ${patientId} at clinic ${clinicId}:`, error);
    return false;
  } finally {
    client.release();
  }
...
export async function expireNoShows(clinicId) {
  try {
    ...
    // console.warn(`Error expiring no-shows for clinic ${clinicId}:`, error);
    return 0;
  }
}
...
export async function getDailyStats(clinicId, date = null) {
  try {
    ...
    // console.warn(`Error getting daily stats for clinic ${clinicId}:`, error);
    return {
